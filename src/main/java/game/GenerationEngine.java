...
public class GenerationEngine {
    ...
    public static short[] generateChunkData(Vector3f chunkPosition, int lod) {
        int chunkSize = Chunk.CHUNK_SIZE / (1 << lod);
        short[] blocks = new short[chunkSize * chunkSize * chunkSize];

        for (int x = 0; x < chunkSize; x++) {
            for (int y = 0; y < chunkSize; y++) {
                for (int z = 0; z < chunkSize; z++) {
                    float globalX = chunkPosition.x * Chunk.CHUNK_SIZE + x * (1 << lod);
                    float globalY = chunkPosition.y * Chunk.CHUNK_SIZE + y * (1 << lod);
                    float globalZ = chunkPosition.z * Chunk.CHUNK_SIZE + z * (1 << lod);
                    ...
                    if (globalY < terrainHeight) {
                        if (globalY == terrainHeight - 1) {
                            if (terrainHeight > BASE_HEIGHT + 10) {
                                blocks[x + y * chunkSize + z * chunkSize * chunkSize] = BlockUtils.create(BlockType.GRASS);
                            } else if (terrainHeight > BASE_HEIGHT - 5) {
                                blocks[x + y * chunkSize + z * chunkSize * chunkSize] = BlockUtils.create(BlockType.SAND);
                            } else {
                                blocks[x + y * chunkSize + z * chunkSize * chunkSize] = BlockUtils.create(BlockType.DIRT);
                            }
                        } else if (globalY < terrainHeight - 1 && globalY > terrainHeight - 8) {
                            blocks[x + y * chunkSize + z * chunkSize * chunkSize] = BlockUtils.create(BlockType.DIRT);
                        } else {
                            blocks[x + y * chunkSize + z * chunkSize * chunkSize] = BlockUtils.create(BlockType.STONE);
                        }
                    } else if (globalY < BASE_HEIGHT - 5 && SimplexNoise.noise(globalX * 0.1f, globalY * 0.1f, globalZ * 0.1f) > 0.6f) {
                        blocks[x + y * chunkSize + z * chunkSize * chunkSize] = BlockUtils.create(BlockType.AIR);
                    } else {
                        blocks[x + y * chunkSize + z * chunkSize * chunkSize] = BlockUtils.create(BlockType.AIR);
                    }
                }
